...
- 단어 원본은 JSON, 정답/시도 기록은 현재 브라우저의 localStorage에 저장합니다. 선택한 단어장·일차·학습 방식과 카드/퀴즈 진행 위치, 정답 화면도 브라우저에 저장되어 새로고침 후 이어집니다. 기기 간 기록 동기화는 없으며 브라우저 데이터를 지우면 학습 기록과 진행 위치도 지워집니다. JSON 문제 내용이 바뀐 범위의 진행 위치는 새로 시작합니다.
